...
import { Printer, Save, CheckCircle, CreditCard, Edit2, Trash2 } from 'lucide-react';
...
export default function Cheques() {
  ...
  const loadTemplate = (bankName) => {
    ...
    if (saved[`${bankName}_Chèque`]) {
      setTemplate(saved[`${bankName}_Chèque`]);
    } else {
  ...
  const getBankDomain = (bankName) => {
    const map = { 'CIH BANK': 'cihbank.ma', 'Banque Populaire': 'groupebanquepopulaire.ma', 'Attijariwafa Bank': 'attijariwafabank.com', 'BMCE Bank': 'bankofafrica.ma', 'Société Générale': 'sgmaroc.com', 'Crédit du Maroc': 'creditdumaroc.ma' };
  ...
  const saveCheck = (status = 'Brouillon') => {
    const newCheck = {
      id: `CHQ-${formData.checkNum || Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`,
      ...formData,
      status,
  ...
  const deleteCheck = (id) => {
    if(window.confirm('Supprimer ce chèque de l\'historique ?')) {
      const allDocs = JSON.parse(localStorage.getItem('emittedDocs') || '[]');
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <header className="page-header hide-on-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p className="page-subtitle">Émission et historique des chèques (Normes BAM).</p>
        </div>
        {!showForm ? (
          <button className="btn-primary" onClick={() => setShowForm(true)}><CreditCard size={18} /> Émettre un Chèque</button>
        ) : (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(400px, 1fr) 1.2fr', gap: '2rem', alignItems: 'start', marginBottom: '2rem' }} className="hide-on-print">
          <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', borderTop: '4px solid var(--accent-primary)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
               <h2 style={{ fontSize: '1.2rem', fontWeight: 600, color: '#fff', margin: 0 }}>Détails du chèque</h2>
            ...
            {carnets.length > 0 ? (
              <div>
                <label style={lbl}>Carnet de Chèques actif</label>
              ...
              <div style={{ color: 'var(--warning)', fontSize: '0.85rem' }}>⚠️ Aucun carnet de chèques ouvert trouvé. Allez dans "Banques".</div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <div>
                ...
                <select name="bank" value={formData.bank} onChange={handleChange} style={{ width: '100%' }}>
                  <option>CIH BANK</option><option>Banque Populaire</option><option>Attijariwafa Bank</option><option>BMCE Bank</option><option>Société Générale</option><option>Crédit Agricole du Maroc</option>
                </select>
              </div>
            </div>
            ...
            <div><label style={lbl}>Observation</label><input name="observation" value={formData.observation} onChange={handleChange} style={{ width: '100%' }} /></div>
          </div>
            ...
            <div style={checkBox} className="printable-check">
              <div style={{ position: 'relative', width: '100%', height: '100%' }}>
                ...
                <div style={{ position: 'absolute', top: 0, right: 0, fontSize: '0.8rem', color: '#475569', fontWeight: 600, letterSpacing: '0.05em' }}>
                  N° {String(formData.checkNum).padEnd(6, ' ')}
                </div>
                ...
                <div style={{ position: 'absolute', top: '100px', left: 0, right: 0, display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px' }}>
                    <span style={previewLabel}>Payez contre ce chèque la somme de :</span>
                ...
                <div style={{ position: 'absolute', top: '180px', right: '10px', display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
                ...
                <div style={{ position: 'absolute', bottom: '40px', right: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <div style={{ position: 'absolute', bottom: '4px', left: '10px', fontFamily: '"Courier New", Courier, monospace', fontSize: '0.8rem', letterSpacing: '0.15em', fontWeight: 700, color: '#1e293b' }}>
                  ⑆ {String(formData.checkNum).padEnd(7, ' ')} ⑈ {selectedCarnet?.account || '01234 567890 12'} ⑉ 11
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="card hide-on-print">
         <h2 style={{ fontSize: '1.2rem', color: '#fff', marginBottom: '1rem' }}>Historique des Chèques</h2>
         <div className="table-container">
          <table>
            ...
            <tbody>
              {emitted.length === 0 ? (
                <tr><td colSpan="7" style={{ textAlign: 'center', color: 'var(--text-muted)' }}>Aucun chèque émis.</td></tr>
              ) : emitted.map(c => (
                <tr key={c.id}>
                  <td style={{ fontWeight: 600, color: '#fff', fontFamily: 'monospace' }}>{c.checkNum}</td>
                  <td>{c.date}</td>
                  <td style={{ fontWeight: 500 }}>{c.payee}</td>
                  <td style={{ color: 'var(--text-muted)' }}>{c.bank}</td>
